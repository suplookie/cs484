...
def hw2_testcase():
    ...
    ## Setup
    test_image = cv2.imread('../data/cat.bmp') / 255.0
    test_image = cv2.resize(test_image, dsize=None, fx=0.7, fy=0.7, )

    result_dir = '../result/ans'
    ...
    identity_image = cv2.filter2D(test_image, -1, cv2.flip(identity_filter, -1))
    ...
    blur_image = cv2.filter2D(test_image, -1, cv2.flip(blur_filter, -1))
    ...
    large_blur_image = cv2.filter2D(test_image, -1, cv2.flip(large_1d_blur_filter, -1))
    large_blur_image = cv2.filter2D(large_blur_image, -1, cv2.flip(large_1d_blur_filter.T, -1))  # notice the transpose operator
    ...
    sobel_image = cv2.filter2D(test_image, -1, cv2.flip(sobel_filter, -1))
    ...
    laplacian_image = cv2.filter2D(test_image, -1, cv2.flip(laplacian_filter, -1))
    ...
    cv2.imwrite(os.path.join(result_dir, 'high_pass_image.jpg'), (laplacian_image + 0.5) * 255)
